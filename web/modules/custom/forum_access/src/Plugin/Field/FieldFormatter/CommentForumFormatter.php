...
namespace Drupal\forum_access\Plugin\Field\FieldFormatter;
...
/**
 * Provides a forum comment formatter.
 *
 * @FieldFormatter(
 *   id = "comment_forum",
 *   module = "forum_access",
 *   label = @Translation("Forum Access comments"),
 *   field_types = {
 *     "comment"
 *   },
 *   quickedit = {
 ...
 * )
 */
class CommentForumFormatter extends CommentDefaultFormatter implements ContainerFactoryPluginInterface {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    if ($status != CommentItemInterface::HIDDEN && empty($entity->in_preview) &&
      ...
      if ($this->currentUser->hasPermission('access comments') || $this->currentUser->hasPermission('administer comments')) {
        ...
        if ($entity->get($field_name)->comment_count || $this->currentUser->hasPermission('administer comments') || forum_access_access('update', $tid) || forum_access_access('delete', $tid)) {
